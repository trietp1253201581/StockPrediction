...
class LocalAttention(nn.Module):
    ...
    def forward(self, x):
        seq_len, batch_size, embed_dim = x.shape  # Đọc đúng kích thước
...
class StandardTransformerModel(BasePytorchModel):
    ...
    def forward(self, x, mask=None, key_padding_mask=None):
        x = self.input_linear(x) # (batch, window, d_model)
        x = x.transpose(0, 1) # (window, batch, d_model)
        x = self.pos_encoder(x)
        
        out = self.encoder(x, mask=mask, key_padding_mask=key_padding_mask)
        out = out[-1, :, :]
        out = self.fc(out) # (batch_size, outdim*ndays)
